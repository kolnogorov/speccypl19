...
; with additional size-efficient speed optimizations by introspec ("Life on Mars" version)
; 202 bytes long and is always faster than "mega" decompressor (by about 3% on average)
; -----------------------------------------------------------------------------
; Parameters:
...
dzx7l_reload:			ld	a, (hl)
				...
				DUP	7				; increasing this number speeds things up a little (max allowed is 7)
				ldi
				add	a
				jr	c, dzx7l_process_ref
				EDUP
...
dzx7l_main_loop:		DUP	3				; the more the better, but it may/will break down some JR optimizations
				add	a
				jr	z, dzx7l_reload
				jr	c, dzx7l_process_ref
...
dzx7l_reload_size1:		ld	a, (hl)
				...
				rla
				jp	c, dzx7l_len_value_done
				DUP	4				; values above 1 speed things up slightly, but not by much - not really worth it
				inc	d
				add	a
				jp	c, dzx7l_len_value_loop
				EDUP
